...
int main(int argc, char *argv[])
{
	...
	*from, *host, *name, *type, *auth, *class;
    ...
    if ((argc - i) < 2) {
	(void) sprintf(temp,
		       "Usage: %s [ -a | -i ] <file> [to] <someone>",
		       arg0);
	(void) putline(2, temp);
	(void) sprintf(temp, "          [ -n name ] [ -c class ]");
	(void) putline(2, temp);
	if (uftcflag & UFT_VERBOSE)
	    return 0;
	else
