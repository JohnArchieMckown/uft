...
int uftdstat(int sock, char *zlda)
{
    ...
    if (tf >= 0)
	(void) putline(tf, buff);
/*  if (cf >= 0) (void) putline(cf,buff);  */
}
...
int main(int argc, char *argv[])
{
    ...
    if (n < 0) {
	(void) sprintf(temp, "522 spool directory unavailable (%d).",
		       errno);
 ...
 */
	if (abbrev("USER", p, 1)) {
	    ...
	    *p = 0x00;
	    ...
	    if (cf < 0) {
		(void) sprintf(temp, "534 %d; meta file error.", errno);
		(void) uftdstat(1, temp);
		return cf;
	    }
 ...
 */
	if (abbrev("DATA", p, 3)) {
	    ...
	    (void) sprintf(temp, "213 %d; received %d bytes of data.", i,
			   i);
	    (void) uftdstat(1, temp);
	    uftfile0.size += i;
	    continue;
 ...
 */
	...
	if (abbrev("TYPE", p, 2) |
	    ...
	    abbrev("DISTRIBUTION", p, 4) | abbrev("TITLE", p, 2)) {
	    /*  put this variable into the control file  */
	    (void) sprintf(temp, "%s='%s'", p, q);
	    if (tf >= 0)
		(void) putline(tf, temp);
