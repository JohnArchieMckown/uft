...
#include "uftcnote.h"
#include <time.h>
//#include "uft_putline.c"
//#include "uft_getline.c"
#include <ctype.h>
...
int s, r;
{
    char temp[256], boundary[256], *name, *p, *verb, *args,
            tstr[64], *day, *mon, *dd, *tttt, *yyyy, meta[256], *from, *targ;
    int i, q;
    time_t t0, t1;
    ...
    *p++ = 0x00;
    ...
    while (*p != 0x00 && *p != ' ' && *p != '\n')
        p++;
    *p++ = 0x00;
    (void) sprintf(temp, "Date: %s, %s %s %s %s", day, dd, mon, yyyy,
            tttt);
    (void) uft_putline(q, temp);
    ...
    (void) uft_putline(q, "# For large files, get your local");
    (void) uft_putline(q, "# sysadmin to install a UFT server.");

    (void) uft_putline(r, "1XX");
    (void) uft_getline(s, temp, 256);
    p = temp;
    verb = p;
    while (*p != 0x00 && *p != ' ' && *p != '=') {
        *p = toupper(*p);
