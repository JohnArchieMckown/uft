...
{				/*  vector of replacement tokens  */
    ...
    if (msgf)
	(void) sprintf(temp, "%s.msgs", msgf);
    else
	(void) sprintf(temp, ".msgs");
    msgfile = dfopen(temp, "r");
    if (msgfile == NULL) {
	punt[1] = msgf;
	if (msgf)
	    return msgread(NULL, NULL, 813, 'E', 2, punt);
	else
	    return "*813";
    }

    if (msgf)
	(void) strncpy(msg1, msgf, 3);
    else
	msg1[0] = 0x00;
    if (msgu)
	(void) strncpy(msg2, msgu, 3);
    else
	msg2[0] = 0x00;
	...
	punt[1] = temp;
	if (msgf)
	    return msgread(NULL, NULL, 814, 'E', 2, punt);
	else
	    return "*814";
    }
 ...
 */
char *msgread(char *msgf,	/*  message file (short, basename), APPLID  */
	      char *msgu,	/*  "caller" or user  */
	      int msgn,		/*  message number  */
	      char msgl,	/*  message level byte  */
	      int msgc,		/*  count of replacement tokens  */
...
int main(int argc, char *argv[])
{
	...
	(void) sprintm(buffer, applid, caller, atoi(argv[n]), level, msgc,
		       msgv);
