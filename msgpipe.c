...
main(int argc, char *argv[]) {
    ...
    errno = 0;
    fd = open(pipefn, O_WRONLY | O_NDELAY);
    ...
    while (1) {
        ...
        while (1) {
            ...
            switch (type[0]) {
                case 'E':
                case 'I':
                    (void) strncpy(outbuf, buffer, 4096);
                    break;
                case 'M':
                case 'W':
                case 'S':
                case 'U':
                default:
                    sprintf(outbuf, "From %s(%s): %s", host, from, buffer);
                    /*			    sprintf(outbuf, 
                                                    "From %s@%s: %s",from,host,buffer);   */
            }
            if (uft_putline(1, outbuf) < 0)
                break;
        }
        (void) close(fd);
    }
